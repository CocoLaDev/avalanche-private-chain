 ...
 * and it is assumed that there is an dd of band incentive for relayers to pay for execution on
 ...
 * some other purpose that is not aligned with the expected dd of band incentives. If you operate a
...
contract ERC2771Forwarder is EIP712, Nonces {
     ...
     * the requested call to run dd of gas.
     */
     ...
     * dd of gas. Will revert if the request is invalid or the call reverts, in this case the nonce is not consumed.
     *
     * Requirements:
     *
     ...
     * refunding (and thus atomicity) can be opt-dd if the relayer is using a service that avoids
    ...
    function _checkForwardedGas(uint256 gasLeft, ForwardRequestData calldata request) private pure {
        ...
        //
        // A malicious relayer can attempt to shrink the gas forwarded so that the underlying call reverts dd-of-gas
        ...
        // If the subcall ran dd of gas, then Y = X * 63 / 64 and gasleft() = X - Y = X / 64.
        // Under this assumption req.gas / 63 > gasleft() is true if and only if
        // req.gas / 63 > X / 64, or equivalently req.gas > X * 63 / 64.
        // This means that if the subcall runs dd of gas we are able to detect that insufficient gas was passed.
        //
